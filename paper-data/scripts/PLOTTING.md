...
|---|---|---|
| `paper/paper_tpch_btree_headline.pdf` | headline.csv | 1×4 row (q3, q5, q3i, q5i btree). bg=2 only, structures S1–S4. **5L cell only** (largest data, low memory pressure) — each panel is a single group of 4 coloured bars (S4/S1/S2/S3 in legend order). Y = seconds/query, **linear** anchored at 0. **Y-cap convention:** the axis ceiling is `second-tallest bar × 1.15 × 1.08 ≈ 1.24×`, so the single tallest bar (usually Base-Hash/S4, often an order of magnitude taller) is **clipped to the top border and annotated with its true value** above the axis — disclose this cap in the figure caption so a glance at bar height isn't read as the full S4-vs-S3 ratio. **Error bars = inter-quartile range over the reps** (`ms_q25`/`ms_q75` from `aggregate_ms_per_query`), drawn on every in-frame bar; suppressed on the clipped over-cap bar (its true value is the printed annotation). Shared legend above. Designed to stack vertically with the lsm sibling. |
...
| `plot_lsm_s3_vs_s2_diagnostics.py` | `2026-05-24-a-ssd/summary/{headline,diagnostics}.csv` + `2026-05-18-b/summary/{headline,diagnostics}.csv` | Emits two figures from the same data. (1) `lsm_s3_vs_s2_diagnostics.{pdf,png}` — 1-column two-panel grouped-bar figure of **S3 / S2 ratio** on RocksDB at c0/bg=2; top = SSD, bottom = HDD; three bars per query (q3, q5, q3i, q5i) = `ms/tx`, `sst_read_us`, `cpu_cycles`; log y, dashed line at 1.0. (2) `lsm_s3_vs_s2_correlation.{pdf,png}` — 2×2 scatter: rows = SSD/HDD, cols = `sst_read` vs `ms/tx` \| `cpu_cycles` vs `ms/tx`; 4 points per panel; marker by family (TPC-H = circle, TPC-Hi = diamond), colour by query; dashed `y=x` diagonal so deviation from the diagonal flags counters that over- or under-explain latency. Both figures cross-tag; no `_ssd`/`_hdd` suffix. Explains the q3i/q5i regression discussed in `PAPER_EDITS.md` (Edit 1). |
