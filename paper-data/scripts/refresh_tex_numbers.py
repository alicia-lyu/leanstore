...
_DEFAULT_HEADLINE_TAG = "2026-05-29-rep0-10L"
_DEFAULT_REFRESH_TAG = "2026-05-30-refresh-10L"
_DEFAULT_DBT_TAG = "2026-05-24-dbtoaster"

# Module-level variables set by main() after argument parsing.
# All loader functions read through these so the entire script is
# parameterizable from the command line without touching call sites.
REPO: Path = _REPO
HEADLINE_TAG: str = _DEFAULT_HEADLINE_TAG
REFRESH_TAG: str = _DEFAULT_REFRESH_TAG
DBT_TAG: str = _DEFAULT_DBT_TAG

DEFAULT_TEX_DIR = _REPO.parent.parent / "merged_index_interesting_orderings" / "sections"
...
def main():
    ...
    parser.add_argument("--tex-dir", type=Path, default=DEFAULT_TEX_DIR,
                        help="directory where experiment_numbers.{json,tex} are written")
    parser.add_argument("--dry-run", action="store_true",
                        help="print output to stdout without writing files")
    parser.add_argument("--paper-data-root", type=Path, default=None,
                        metavar="DIR",
                        help="root that contains the tag sub-directories. "
                             "Default: the in-repo paper-data/ directory. "
                             "Pass /results when running inside the artifact container.")
    parser.add_argument("--headline-tag", default=None, metavar="TAG",
                        help=f"tag dir name for the headline sweep "
                             f"(default: {_DEFAULT_HEADLINE_TAG})")
    parser.add_argument("--refresh-tag", default=None, metavar="TAG",
                        help=f"tag dir name for the refresh sweep "
                             f"(default: {_DEFAULT_REFRESH_TAG})")
    parser.add_argument("--dbt-tag", default=None, metavar="TAG",
                        help=f"tag dir name for the DBToaster results "
                             f"(default: {_DEFAULT_DBT_TAG})")
    args = parser.parse_args()

    # Apply overrides to module-level variables so all loader functions pick
    # them up without needing explicit parameter threading.
    global REPO, HEADLINE_TAG, REFRESH_TAG, DBT_TAG
    if args.paper_data_root is not None:
        REPO = args.paper_data_root.resolve()
    if args.headline_tag is not None:
        HEADLINE_TAG = args.headline_tag
    if args.refresh_tag is not None:
        REFRESH_TAG = args.refresh_tag
    if args.dbt_tag is not None:
        DBT_TAG = args.dbt_tag
    ...
    tex_lines = [
        ...
        "% Regenerate: python3 paper-data/scripts/refresh_tex_numbers.py [--paper-data-root ...]",
        "% Do not edit by hand.",
        "",
    ]
    for spec in SPECS:
