#!/usr/bin/env python3
"""Walk paper-data/<tag>/raw/<binary>/c0-bg2-r<rep>/ dirs and produce
TPut.s<sx>.csv from each structure<sx>.log. Works across reps. Idempotent:
re-running overwrites existing TPut.s<sx>.csv with the current log content.
"""
import csv
import re
import subprocess
...
TAG_DIR = Path("/users/alicial/leanstore/paper-data/2026-05-28-rep0-5L")
REP_RE = re.compile(r"c0-bg2-r(\d+)")
STRUCT_RE = re.compile(r"structure(\d+)\.log")
SCALES = {"lsm": "3850", "btree": "1550"}
DRAM = "1.0"
...
def main():
    ...
    # Update commit + reps + runs_ok in-place; leave rest untouched if non-trivial
