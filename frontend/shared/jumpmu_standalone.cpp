// Standalone jumpmu implementation for macOS (ROCKSDB_ONLY) builds.
// On Linux these come from backend/leanstore/utils/JumpMU.cpp via libleanstore.a.
...
void jump()
{
   ...
   if (de_stack_counter > c_c_stacks_counter) {
      int begin = de_stack_counter - 1;
      int till = c_c_stacks_counter;
      assert(begin >= 0);
      assert(till >= 0);
      in_jump = true;
