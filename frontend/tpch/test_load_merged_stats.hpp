...
// counts. Used by test_load_merged_{rocksdb,leanstore}.cpp.
//
    ...
    Integer expected_max_orderkey)
{
      ...
      std::visit(
          [&](auto&& key) {
             ...
             if constexpr (std::is_same_v<K, orders_t::Key>) {
                ...
                current_lineitems = 0;
                min_orderkey = std::min(min_orderkey, key.o_orderkey);
                max_orderkey = std::max(max_orderkey, key.o_orderkey);
   ...
   const long total = orders_count + lineitems_count;
   const double avg_li_per_order =
       orders_count > 0 ? static_cast<double>(lineitems_count) / orders_count : 0.0;
   ...
   std::cout << "\n=== MI[0] distribution (scale_factor=" << FLAGS_tpch_scale_factor
             << ") ===\n";
   std::cout << pass(orders_count == expected_orders_count)
             << " orders count: " << orders_count
             << "  (expected " << expected_orders_count << ")\n";
   std::cout << pass(lineitems_count >= expected_li_min && lineitems_count <= expected_li_max)
             << " lineitems count: " << lineitems_count
             << "  (expected " << expected_li_min << "-" << expected_li_max
             << ", typical ~" << (expected_orders_count * 4) << ")\n";
   std::cout << pass(total == orders_count + lineitems_count)
             << " total MI[0] keys: " << total << "\n";
   std::cout << pass(max_orderkey == expected_max_orderkey)
             << " orderkey range: [" << min_orderkey << ", " << max_orderkey
   ...
   std::cout << "       MI[0] size: " << merged_ol.size() << " MiB\n";
}
