...
- **Result — size-stable PAIRS/s (TPC-H unit, = 5L `pair_tps`):** S1 20.5k, **S3 18.5k**, S2 10.4k, S4 27.3k vs **DBToaster 13.8k** (`summary/pair_vs_dbtoaster.csv`). On the pair, merged **S3 (18.5k) and split S1 (20.5k) beat DBToaster (13.8k); only view S2 (10.4k) trails it** — DBToaster's fast inserts (RF1 86.3k) are offset by slow deletes (RF2 16.4k → pair 13.8k; its own harness prints "pair orders/s 27.6k" = 2× counting both orders). Within LeanStore the §5.4 ordering holds in-memory (**S3 ≈ S1 ≫ S2**; split edges merged slightly with everything resident). Attribution (phase ops/s): DBToaster wins RF1 (86.3k vs S3 31.1k — codegen, no TX/WAL/MVCC), LeanStore wins RF2 (S3 46.4k vs 16.4k). Real lever is memory (DBToaster peak 8.25 GiB, OOMs below working set; LeanStore runs the same views at 0.4–1.0 GiB by spilling, per 5L). In-memory confirmed: S4 24.8k pair/s agg vs 5L-SSD 5.4k. Caveat: `update_size=1` = one TX/order; batched-TX would narrow the RF1 gap (not measured).
