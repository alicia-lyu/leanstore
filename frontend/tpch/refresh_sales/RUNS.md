...
- **Result (median per-iteration phase orders/s):** S1 34.9k/49.7k, S2 17.8k/25.0k, **S3 31.1k/46.4k**, S4 43.8k/73.9k (RF1/RF2) vs DBToaster RF1 86.3k / RF2 16.4k. Within LeanStore the §5.4 ordering holds in-memory (**S3 ≈ S1 ≫ S2**). vs DBToaster: it wins RF1 inserts (~2.8× S3 — codegen, no TX/WAL/MVCC), LeanStore wins RF2 deletes; the real contrast is memory (DBToaster peak 8.25 GiB, OOMs below working set; LeanStore runs the same views at 0.4–1.0 GiB by spilling, per the 5L run). In-memory confirmed: S4 54.6k pair/s vs 5L-SSD 5.4k. Caveat: `update_size=1` = one TX/order; batched-TX would narrow the RF1 gap (not measured).
