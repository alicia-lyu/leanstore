...
class Experiment:
    class DBConfig:
        def __init__(self, executable, target_gib, selectivity, included_columns, outer_join):
        ...
        def check_image_size(self, write_enabled, returncode):
            if self.archive_image.exists() and not write_enabled and returncode == 0:
    ...
    class RuntimeConfig:
        def __init__(self, dbconfig, dram_gib, read_percentage, scan_percentage, write_percentage, duration):
            self.dbconfig: Experiment.DBConfig = dbconfig
        ...
        def get_csv_prefix(self):
            return f"{self.dbconfig.log_dir}/{self.timestamp}"
        ...
        def two_phase(self):
            return self.persist and self.dram_gib < self.target_gib * 2
        
        def get_flags(self):
            return [
                f"--csv_path={self.get_csv_prefix()}", "--csv_truncate=true",
        ...
        def get_2p_flags(self):
            return [
                ...
                f"--persist_file={self.dbconfig.build_dir / "leanstore.json"}", f"--dram_gib={self.dram_gib}",
                f"--read_percentage={self.read_percentage}", f"--scan_percentage={self.scan_percentage}", f"--write_percentage={self.write_percentage}"
            ]
    ...
    def __init__(self, executable, dram_gib, target_gib, read_percentage, scan_percentage, write_percentage, selectivity, included_columns, duration, outer_join):
        self.dbconfig = self.DBConfig(executable, target_gib, selectivity, included_columns, outer_join)
        self.runtimeconfig = self.RuntimeConfig(self.dbconfig, dram_gib, read_percentage, scan_percentage, write_percentage, duration)
    ...
    def run_command(self, extra_flags=None):
        ...
        with open(self.dbconfig.log_dir / "config.csv", "w") as config_f:
        ...
        if not self.runtimeconfig.persist or result.returncode != 0:
            self.dbconfig.cleanup_image()
        else:
            self.dbconfig.copy2archive()
